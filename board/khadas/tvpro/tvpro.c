...
static void select_fdtfile(void)
{
	...
	case 0x100000000:
		if (cpu_id.chip_rev == 0x0a || cpu_id.chip_rev == 0x0b) {
			env_set("chip_recv", "b");
			env_set("fdtfile", "amlogic/t7-" CONFIG_DEFAULT_DEVICE_TREE "-4g.dtb");
		} else if (cpu_id.chip_rev == 0x0c) {
			env_set("chip_recv", "c");
			env_set("fdtfile", "amlogic/t7c-" CONFIG_DEFAULT_DEVICE_TREE "-4g.dtb");
		}
		break;
	...
	case 0x200000000:
		if (cpu_id.chip_rev == 0x0a || cpu_id.chip_rev == 0x0b) {
			env_set("chip_recv", "b");
			env_set("fdtfile", "amlogic/t7-" CONFIG_DEFAULT_DEVICE_TREE "-8g.dtb");
		} else if (cpu_id.chip_rev == 0x0c) {
			env_set("chip_recv", "c");
			env_set("fdtfile", "amlogic/t7c-" CONFIG_DEFAULT_DEVICE_TREE "-8g.dtb");
		}
		break;
	...
	default:
		if (cpu_id.chip_rev == 0x0a || cpu_id.chip_rev == 0x0b) {
			env_set("chip_recv", "b");
			env_set("fdtfile", "amlogic/t7-" CONFIG_DEFAULT_DEVICE_TREE "-unsupported.dtb");
		} else if (cpu_id.chip_rev == 0x0c) {
			env_set("chip_recv", "c");
			env_set("fdtfile", "amlogic/t7c-" CONFIG_DEFAULT_DEVICE_TREE "-unsupported.dtb");
		}
		break;
	}
}
...
int board_late_init(void)
{
	...
	/*
	 * Set the default value of mipi_lcd_exist to prevent
	 * the subsequent code from getting the value of mipi_lcd_exist as NULL,
	 * which may cause a crash
	 * eg: func do_edid in file cmd/amlogic/cmd_hdmitx21.c
	 */
	char *mipi_lcd_exist = NULL;
	mipi_lcd_exist = env_get("mipi_lcd_exist");
	if(!mipi_lcd_exist) {
		printf("env mipi_lcd_exist not found, set to 0 default\n");
		env_set("mipi_lcd_exist", "0");
	}
